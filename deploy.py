...
def send_gmail(user, pwd, recipient, subj, txt):
    import smtplib
    import ssl

    smtp_server = "smtp.gmail.com"
    sender = user
    rcpt = recipient if isinstance(recipient, list) else [recipient]
    theme = subj
    text = txt

    # Prepare actual message
    message = """From: %s\nTo: %s\nSubject: %s\n\n%s
        """ % (sender, ", ".join(rcpt), theme, text)
    ...
    #     server_ssl.sendmail(sender, rcpt, message)
    #     server_ssl.close()
